...
var App = function () {
  ...
  var handleIframeContent = function () {
    ...
    var height = App.getViewPort().height - ($footer.outerHeight() ||0) - ($header.outerHeight() || 0);
    if ($tabs.is(":visible")) {
  ...
  return {
    ...
    blockUI: function (options) {
      ...
      if (options.animate) {
        ...
        html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '') + '"><img src="/imgs/loading/loading-spinner-grey.gif"></div>';
      } else if (options.textOnly) {
        html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '') + '"><span>&nbsp;&nbsp;' + (options.message ? options.message : 'LOADING...') + '</span></div>';
      } else {
        html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '') + '"><img src="/imgs/loading/loading-spinner-grey.gif"><span>&nbsp;&nbsp;' + (options.message ? options.message : 'LOADING...') + '</span></div>';
      }
...
var addTabs = function (options) {
  ...
  if (findTabPanel(pageId) === null) {
    ...
    //是否允许关闭
    if (options.close) {
      var $i = $("<i class='fa fa-times page_tab_close' style='cursor: pointer' onclick='closeTab(this);'></i>").attr(pageIdField, pageId).appendTo($title);
      // title += ' <i class="fa fa-times page_tab_close" style="cursor: pointer;" data-id="' + pageId + '" onclick="closeTab(this)"></i>';
    }
...
(function ($) {
  $.fn.sidebarMenu = function (options) {
    ...
    function init ($menu_ul, data, level) {
      $.each(data, function (i, item) {
        ...
        if ((level == 0 && item.children.length == 0)) { 
          var li = $('<li class="nav-item" data-level="' + level + '"></li>');
          $a = $('<a></a>');
          //图标
          var $icon = $('<i></i>');
          ...
          var li = $('<li class="nav-item" data-level="' + level + '"></li>');
          $a = $('<a  href="#" data-toggle="collapse" data-target="#collapse' + item.id + '" aria-expanded="true" aria-controls="collapse' + item.id + '"></a>');
          //图标
          var $icon = $('<i></i>');
          $icon.addClass(item.icon);
        ...
        if (item.children && item.children.length > 0) {
          ...
          var menus_div = $('<div id="collapse' + item.id + '" data-parent="#accordionSidebar" aria-labelledby="heading' + item.id + '"></div>');
          menus_div.addClass('collapse');
          var menus = $('<div class="bg-white py-2 collapse-inner rounded"></div>')

          init(menus, item.children, level + 1);
          menus_div.append(menus);
          li.append(menus_div);
          $menu_ul.append(li);
        }
        else {
    ...
    //另外绑定菜单被点击事件,做其它动作
    $menu_ul.on("click", "li.nav-item a", function () {
      var $a = $(this);
...
module.exports = { App, context, createCallback, getPageId, findTabPanel, addTabs, closeTab, scrollTabRight, closeCurrentTab, closeOtherTabs, refreshTab, closeTab, scrollTabLeft }